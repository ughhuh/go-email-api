...
var queries_map = map[string]string{
	...
	"deleteMsgByMsgId":      `delete from emails where message_id in ANY($1)`,
}

func connectToDb() *sql.DB {
	connStr := fmt.Sprintf("user=%s password=%s dbname=%s sslmode=%s",
		viper.GetString("DB_USER"), viper.GetString("DB_SECRET"), viper.GetString("DB_NAME"), viper.GetString("ssl_mode"))
